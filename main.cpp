...
//				cout << i->first.p2.x << "," << i->first.p2.y << endl;
	...
	for(int i=0;i<diagonals.size();i++){
	 ...
	 DCEL(n, n, vertices, diagonals, argc, argv);
	// cout<<face_table.size()<<" this is the no of faces\n";
...
//triangulate
	...
	vector<Point> vertices_ccw_t(n_t);
	
	cout<<n_t<<" this is no of vertices";
	for(int i=0;i<n_t;i++)
	{
		cin >> vertices_sorted_toy_t[i].x >> vertices_sorted_toy_t[i].y;
		vertices_sorted_toy_t[i].key=i;
		vertices_ccw_t[i].x=vertices_sorted_toy_t[i].x;
		vertices_ccw_t[i].y=vertices_sorted_toy_t[i].y;
		vertices_ccw_t[i].key=vertices_sorted_toy_t[i].key;
	}
	...
	for(int j=2;j<n_t-1;j++)
	{
		if(AreOnOppSide(vertices_sorted_toy_t[j],st_t.top(),left,right))
		{
			...
			while(st_t.size()>1)//ppop and make diagonals except the last one
			{
				...
				st_t.pop();
				diagonals.push_back({temp_t.key,vertices_sorted_toy_t[j].key});
			}
		...
		else// same side
		{
			...
			while(!st_t.empty())
			{
				...
				if(isonleft)//if point on left chain
				{
					...
					{
						diagonals.push_back({vertices_sorted_toy_t[j].key,above_temp_t.key});
					}
					else
						break;
				}
	...
	while(st_t.size()>1)
	{
		...
		diagonals.push_back({random_top.key,vertices_sorted_toy_t[n-1].key});
		st_t.pop();
	}
	 st_t.pop();
